    ...
    stages {
        ...
        stage('Sqoop Import') {
            steps {
                ...
                sshagent(credentials: ['hadoop-ssh-key']) {
                    sh """
                        ssh -o StrictHostKeyChecking=no ${SSH_USER}@${HADOOP_NODE} '
                            sqoop import \
        ...
        stage('Create Hive Tables') {
            steps {
                ...
                sshagent(credentials: ['hadoop-ssh-key']) {
                    sh """
                        scp -o StrictHostKeyChecking=no src/hive_table.sql ${SSH_USER}@${HADOOP_NODE}:/tmp/hive_table.sql
                        ssh -o StrictHostKeyChecking=no ${SSH_USER}@${HADOOP_NODE} 'hive -f /tmp/hive_table.sql'
                    """
                }
            }
        }
        ...
        stage('Verify Tables') {
            steps {
                ...
                sshagent(credentials: ['hadoop-ssh-key']) {
                    sh """
                        ssh -o StrictHostKeyChecking=no ${SSH_USER}@${HADOOP_NODE} 'hive -e "USE yamini_tfl_proj; SHOW TABLES;"'
                    """
                }
            }
        }
